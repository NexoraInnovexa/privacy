...
logger.info("🚀 Starting Privacy Audit Backend (using Hugging Face API)...")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # optionally restrict
...
HF_TOKEN = os.getenv("HF_TOKEN") # ⬅️ paste your token here
...
def generate_plain_text(input_text: str):
    ...
    try:
        payload = {
            "inputs": f"Explain privacy risks in plain language:\n{input_text}"
        }
...
def analyze(data: dict):
    logger.info(f"🔍 /analyze called with: {data}")
    try:
...
async def audit_mvp(data: dict):
    logger.info("🔍 /audit (dynamic) called")
    try:
        os_apps = data.get("os_apps", [])
        browser_exts = data.get("browser_extensions", [])
        account_apps = data.get("account_apps", [])
